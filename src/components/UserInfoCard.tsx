...
export const UserInfoCard: React.FC<UserInfoCardProps> = ({
  ...
  // 数量优先从 hoverData.interact_info 取（UserInfoCard 契约），
  // 兼容 realGetUserInfo 返回的 user 对象（mapBlogInfo 填的 follow_count/fans_count/post_count）
  const follows = hoverData?.interact_info?.follows ?? (hoverData as any)?.follow_count ?? 0;
  const fans = hoverData?.interact_info?.fans ?? (hoverData as any)?.fans_count ?? 0;
  const posts = hoverData?.interact_info?.posts ?? (hoverData as any)?.post_count ?? 0;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 10 }}>
        ...
        {!loading && !error && hoverData && (follows > 0 || fans > 0 || posts > 0) && (
          <>
